	...
	proto.framed_photo = function(width, height, frame_width_perc, frame_color, shadow_opacity, shadow_offset) {
	  ...
	  preprocess_cb = __bind(function(callback) {
	    return this.autoOrient().background("transparent").filter("Bessel").gravity("Center").background(frame_color).enhance().size(__bind(function(err, value) {
	      ...
	      this.sharpen(2,border_size).extent(orig_w + border_size, orig_h + border_size).background("white").gravity("SouthEast").extent(orig_w + border_size+1, orig_h + border_size+1).background("transparent").gravity("Center").resize(width, height).rotate("transparent", rot).shadow(shadow_opacity, Math.abs(shadow_offset * 2), shadow_offset, shadow_offset, width*2);
